...
const on = <T>(channel: string, callback: (payload: T) => void) => {
  const fn = (_: unknown, p: T) => callback(p);
...
contextBridge.exposeInMainWorld('maiDesktop', {
  ...
  startTerminal: (id: string, mode: 'mai' | 'install', shell?: string) =>
    ipcRenderer.invoke('terminal:start', { id, mode, shell }),
  ...
  downloadAction: (id: string, action: string) =>
  ...
  updateCli: () => ipcRenderer.invoke('cli:update'),
  onMenuNewTerminal: (cb: () => void) => on('menu:new-terminal', cb),
  onMenuCloseTab: (cb: () => void) => on('menu:close-tab', cb),
  onMenuSelectTab: (cb: (idx: number) => void) => on('menu:select-tab', cb),
  onMenuNextTab: (cb: () => void) => on('menu:next-tab', cb),
  onMenuPrevTab: (cb: () => void) => on('menu:prev-tab', cb),
