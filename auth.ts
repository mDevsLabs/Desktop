...
export function registerAuthRoutes(app: Hono) {
  ...
  app.post('/verify-login', async (c) => {
    try {
      ...
      const sql = getDb();
      const users = await sql`SELECT id, tier FROM users WHERE email = ${email} LIMIT 1`;
      if (users.length === 0) {
  ...
  app.post('/resend-code', async (c) => {
    try {
      ...
      if (result.rows.length > 0) {
        ...
        if (diffMinutes > 9) {
          return c.json({ error: 'Veuillez patienter 1 minute avant de renvoyer un code.' }, 429);
        }
      }
  ...
  app.post('/update-profile', async (c) => {
    try {
      ...
      const { username, email, phone, password, currentPassword, newsletter, notify_limits } =
        await c.req.json();
      const sql = getDb();
      ...
      if (!currentPassword) {
        return c.json(
          {
            error: 'Le mot de passe actuel est obligatoire pour modifier vos informations.',
          },
          400
        );
      }
      ...
      const passMatch = await bcrypt.compare(currentPassword, currentUser[0].password_hash);
      if (!passMatch) {
      ...
      if (email && email.trim()) {
        ...
        if (existing.length > 0) {
          return c.json({ error: 'Cette adresse e-mail est déjà utilisée.' }, 400);
        }
